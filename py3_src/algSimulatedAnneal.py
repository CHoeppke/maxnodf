...
def optimise(NodesA, NodesB, Edges, alpha = 0.998, iters = 12, verbose = True):
    mtx = greedySolver2.greedySolve(NodesA, NodesB, Edges)
    mtx = hill_climb.full_hill_climb(mtx, multithread = True )
    bestMTX = simulatedAnnealing.sim_anneal_opt(mtx, alpha = alpha, iters = iters)
    print("Opt NODF: {}".format(toolbox.nodf(bestMTX)))
    return bestMTX
